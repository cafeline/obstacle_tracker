...
TEST_F(ObstacleTrackerTest, TestVoxelizationAndClustering) {
  ...
  auto inspect_markers = [&](const std::vector<visualization_msgs::msg::Marker> & markers) {
  ...
  inspect_markers(static_obstacles_msg_.markers);
  inspect_markers(dynamic_obstacles_msg_.markers);

  EXPECT_TRUE(found_front_obstacle) << "前方の障害物が検出されませんでした";
  EXPECT_TRUE(found_right_obstacle) << "右方の障害物が検出されませんでした";
}

// テスト3: パラメータ設定の確認
TEST_F(ObstacleTrackerTest, TestParameterConfiguration) {
  auto processing_range = obstacle_tracker_node_->get_parameter("robot_processing_range");
  auto dynamic_threshold = obstacle_tracker_node_->get_parameter("dynamic_cluster_speed_threshold");

  EXPECT_DOUBLE_EQ(processing_range.as_double(), 10.0) << "robot_processing_rangeのデフォルト値が正しくありません";
  EXPECT_DOUBLE_EQ(
    dynamic_threshold.as_double(),
    0.5) << "dynamic_cluster_speed_thresholdのデフォルト値が正しくありません";
}
...
TEST_F(ObstacleTrackerTest, TestMinimumClusterSize) {
  ...
  auto count_cluster_markers = [](const visualization_msgs::msg::MarkerArray & array) {
      return std::count_if(
        array.markers.begin(), array.markers.end(), [](const auto & marker) {
          return marker.action != visualization_msgs::msg::Marker::DELETEALL;
        });
    };

  int total_clusters = count_cluster_markers(static_obstacles_msg_) +
    count_cluster_markers(dynamic_obstacles_msg_);
